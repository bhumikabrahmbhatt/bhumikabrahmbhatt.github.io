		...
		nodes.forEach((node, id) => {
			...
			$tr.insertAdjacentHTML('beforeEnd', node.type != 'parent' ? `
				...
				<td>
			        ...
			        <button class="far fa-edit" data-action="edit" title="Edit file"></button> 
					<button class="far fa-trash-alt btn btn-danger btn-xs" data-action="remove" title="Delete"></button>
					<button class="far fa-arrow-alt-circle-down btn btn-primary btn-xs" data-action="download" title="Download"></button>
				    <button class="fas fa-user-cog btn btn-info btn-xs" data-action="info" title="Privilege"></button>
			    	<button class="fas fa-link" data-action="link" title="Get link"></button> 
				</td>
			`: `<td><span data-href="${node.uri}" data-action="navigate">${node.name}</span></td>
				<td colspan="4">&nbsp;</td>`);
